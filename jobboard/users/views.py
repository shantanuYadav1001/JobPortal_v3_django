    ...
    if request.user.is_employer == True:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                # Save the job instance manually
                job.save()
                # Save the form instance to commit all changes
                form.save()
                return redirect('users:employer_job_list')
        return render(request, 'new/add_job_form.html', {'form': form, 'is_logged_in': is_logged_in, 'is_employer' : is_employer})
    else:
        messages.error(request, 'You do not have permission to access this page.')
        return redirect('users:home')
...
def job_search(request):
    ...
    if query:
        jobs = Job.objects.filter(Q(company_name__icontains=query) 

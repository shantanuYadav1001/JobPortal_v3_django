...
class PostJobForm(forms.ModelForm):
    ...
    job_category = forms.ChoiceField(required=True, choices=Job.JOB_CATEGORIES, widget=forms.Select(attrs={'class': 'form-control'}))
    job_type = forms.ChoiceField(required=True, choices=Job.JOB_TYPE_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))

    #job_description = forms.CharField(required=True, widget=forms.Textarea(attrs={'class': 'form-control'}))
    location = forms.ChoiceField(required=True, choices=LOCATION_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))

    featured_image = forms.ImageField(required=True, widget=forms.FileInput(attrs={'class': 'form-control'}))
    email = forms.EmailField(required=True, widget=forms.EmailInput(attrs={'class': 'form-control'}))
    ...
    logo = forms.ImageField(label='Company Logo' , required=True, widget=forms.FileInput(attrs={'class': 'form-control'}))
    
    last_date_of_application = forms.DateField(required=True, widget=forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}))
    
    salary_min = forms.IntegerField(required=True, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    salary_max = forms.IntegerField(required=True, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    
    experience_min = forms.IntegerField(label='Minimum Experience (years)', required=True, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    experience_max = forms.IntegerField(label='Maximum Experience (years) set 0 for freshers' ,required=True, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    
    status = forms.ChoiceField(choices=STATUS_CHOICES, required=True, widget=forms.Select(attrs={'class': 'form-control'}))
...
class JobApplicationForm(forms.ModelForm):
    ...
    portfolio_links = forms.URLField(
        required=True,
        widget=forms.URLInput(attrs={'class': 'form-control'})
    )
    class Meta:
